...
def makeRES_SIMPLERECT_VER(rm, rmdisp, l, w, h, ddrill, material, R_POW, x_3d=[0, 0, 0], s_3d=[1 / 2.54, 1 / 2.54, 1 / 2.54],  has3d=1, specialfpname="", largepadsx=0, largepadsy=0, specialtags=[]):
	...
	l_slk = left - slk_offset
	h_slk = max(pady,rh_slk) + w_slk
	t_slk = -d_slk / 2

	w_crt = rw_slk/2+padx/2 + 2 * crt_offset
	...
	# create SILKSCREEN-layer
	kicad_mod.append(RectLine(center=[left,top], end=[left+rw_slk,top+rh_slk], layer='F.SilkS'))
	xs1 = rw_slk / 2
	xs2 = rm - padx / 2 - 0.3
	if (xs1 < xs2):
		kicad_mod.append(Line(start=[xs1, 0], end=[xs2, 0], layer='F.SilkS'))
...
if __name__ == '__main__':
	...
	makeRES_RECT_HOR(      25.4,     25,    19,     8,     8,       1,   "Ceramic",      "",         [0.5, 0, 0],                   [4, 4, 4 ],       1)
	makeRES_RECT_HOR(     30.48,     30,    23,     9,     9,     1.2,   "Ceramic",      "",         [0.6, 0, 0],                   [4, 4, 4 ],       1)
	makeRES_RECT_HOR(     45.72,     45,    36,    11,    10,     1.2,   "Ceramic",      "",         [0.9, 0, 0],                   [4, 4, 4 ],       1)
	makeRES_RECT_HOR(     60.96,     60,    50,    14,    13,     1.2,   "Ceramic",      "",         [1.2, 0, 0],                   [4, 4, 4 ],       1)
	makeRES_RECT_HOR(     80.01,     80,    65,    16,    15,     1.2,   "Ceramic",      "",        [1.58, 0, 0],                [3.95, 4, 4 ],       1)
	makeRES_RECT_HOR(   35.0012,     35,    20,   6.6,   6.6,     1.2,   "Ceramic",      "",           [0, 0, 0],       [1/2.54,1/2.54,1/2.54],       0,      "Resistor_Cement_Horizontal_Meggitt-SBC-2",                         ["Meggit", "SBC", "SBC-2"])
	...
	makeRES_VERT(             5,      5,  25.5,    14,    10,     1.2,   "Ceramic",  "5W 7",          [0, 0, 0],       [1/2.54,1/2.54,1/2.54],       0,                 "Resistor_Cement_Vertical_KOA-BGR-5N-7N",      ["Meggitt","KOA","BSR","BGR","BWR","5N","7N"])
	makeRES_VERT(             5,      5,  25.5,    14,    10,     1.2,   "Ceramic",  "5W 7",          [0, 0, 0],       [1/2.54,1/2.54,1/2.54],       0,       "Resistor_Cement_Vertical_LargePads_KOA-BGR-5N-7N",      ["Meggitt","KOA","BSR","BGR","BWR","5N","7N"],            3)
	makeRES_VERT(             5,      5,  25.5,    13,     9,     1.2,   "Ceramic",     "3",          [0, 0, 0],       [1/2.54,1/2.54,1/2.54],       0,                 "Resistor_Cement_KOA-BGR-3N",         ["Meggitt","KOA","BSR","BGR","BWR","3N"])
	makeRES_VERT(             5,      5,  25.5,    13,     9,     1.2,   "Ceramic",     "3",          [0, 0, 0],       [1/2.54,1/2.54,1/2.54],       0,       "Resistor_Cement_LargePads_KOA-BGR-3N",         ["Meggitt","KOA","BSR","BGR","BWR","3N"],                 3)
	...
	makeRES_SIMPLERECT_HOR(     15,    15, 6.5, 2.3,        1,   "1/4",       [0.295, 0, 0],           [0.395, 0.4, 0.4 ],       1)
	makeRES_SIMPLERECT_HOR(     20,    20,  15,   5,        1,    "2",        [0.395, 0, 0],           [0.395, 0.4, 0.4 ],       1)
	makeRES_SIMPLERECT_HOR(     25,    25,  17,   6,        1,    "3",        [0.395, 0, 0],           [0.395, 0.4, 0.4 ],       0)
	makeRES_SIMPLERECT_HOR(     30,    30,  24,   8,      1.2,    "5",        [0.395, 0, 0],           [0.395, 0.4, 0.4 ],       0)
	
	#                          rm, rmdisp,    l,      d, ddrill,   R_POW,    x_3d=[0,0,0],   s_3d=[1/2.54,1/2.54,1/2.54], has3d=1, specialfpname="", largepadsx=0, largepadsy=0
	makeRES_SIMPLECIRC_VER(   5.08,     5,   15,      5,      1,     "2",     [0.2, 0, 0],              [0.4, 0.4, 0.4 ], 0)
	makeRES_SIMPLECIRC_VER(    7.5,   7.5,   24,     10,      1,     "5",     [0.2, 0, 0],              [0.4, 0.4, 0.4 ], 0)
